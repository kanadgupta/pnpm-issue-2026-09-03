...
- pnpm 12's Rust CLI parses patch files with the [`diffy`](https://github.com/bmwill/diffy) crate: [`crates/patching/src/apply.rs`](https://github.com/pnpm/pnpm/blob/v12.3.4/pnpm/crates/patching/src/apply.rs) reads the file and hands the text to `PatchSet::parse` unchanged. `Cargo.toml` asks for `diffy = "0.5.0"` (a semver range, so any `0.5.x`), and [`Cargo.lock`](https://github.com/pnpm/pnpm/blob/v12.3.4/Cargo.lock) resolves that to **0.5.1**; release builds run `cargo build --locked`, and the published `pnpm` 12.3.1 and 12.3.4 binaries both embed `diffy-0.5.1` source paths. `diffy` 0.5.1's `parse_filename` splits the `---`/`+++` header lines at `\n` only, so with CRLF endings the `\r` stays attached to the filename and the parser rejects it as an "invalid char in unquoted filename". `diffy` [0.5.2](https://github.com/bmwill/diffy/blob/0.5.2/CHANGELOG.md) (2026-08-31, [bmwill/diffy#87](https://github.com/bmwill/diffy/pull/87)) strips the `\r`, and is within the range `Cargo.toml` already allows.
